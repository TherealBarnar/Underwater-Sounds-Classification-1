...
def apply_smote(X_train, y_train, k_neighbors=1):
    ...
    # Primo livello: SMOTE sulle subclass
    X_resampled = pd.DataFrame()
    y_resampled = pd.Series(dtype=int)

    for class_label in X_train[class_column].unique():
        X_class = X_train[X_train[class_column] == class_label]
        subclasses = X_class[subclass_column]

        subclass_counts = subclasses.value_counts()
        valid_subclasses = subclass_counts[subclass_counts > 1].index
        mask = subclasses.isin(valid_subclasses)
        X_class = X_class[mask]
        subclasses = subclasses[mask]

        smote = SMOTE(k_neighbors=k_neighbors, random_state=42)
        X_class_resampled, subclasses_resampled = smote.fit_resample(
            X_class.select_dtypes(include=[np.number]), subclasses
        )

        # Accumula risultati
        X_resampled = pd.concat([X_resampled, pd.DataFrame(X_class_resampled)], axis=0)
        y_resampled = pd.concat([y_resampled, pd.Series([class_label] * len(subclasses_resampled))], axis=0)

    # **Assicurati che y_resampled sia numerico (0 o 1)**
    y_resampled = y_resampled.map({'Target': 1, 'Non-Target': 0}).astype(int)

    # Distribuzione delle subclass dopo SMOTE
    print("\nDistribuzione delle subclass DOPO il primo livello di SMOTE:")
    print(pd.Series(subclasses_resampled).value_counts())

    # Secondo livello: SMOTE sulle class principali
    smote = SMOTE(k_neighbors=k_neighbors, random_state=42)
    X_final_resampled, y_final_resampled = smote.fit_resample(X_resampled, y_resampled)

    # Conversione finale delle etichette
    y_final_resampled = pd.Series(y_final_resampled).astype(int).to_numpy()

    # Distribuzione finale delle classi
    print("\nDistribuzione delle class DOPO SMOTE:")
    print(pd.Series(y_final_resampled).value_counts())
